...
//      .show()
    ...
    val resSchema = StructType(
      List(
        ...
        StructField("avg(seconds)", DoubleType, true),
        StructField("stddev_samp(seconds)", DoubleType, true)
      )
    )
    //转成RDD处理NAN
    val newRdd: RDD[Row] = frame.rdd.mapPartitions(iter => {
      val list: ArrayBuffer[Row] = new ArrayBuffer[Row]()
      while (iter.hasNext) {
        val row: Row = iter.next()
        val str1 = Option(row.getAs[String]("borough")).getOrElse("")
        val str2 = Option(row.getAs[Double]("avg(seconds)")).getOrElse(0.0)
        var str3 = Option(row.getAs[Double]("stddev_samp(seconds)")).getOrElse(0.0)
        if(str3.isNaN){
          str3=0.0
        }
        val schemaData: GenericRowWithSchema = new GenericRowWithSchema(Array(str1, str2, str3), resSchema)
        list.append(schemaData)
      }
      list.iterator
    })
    //组装成DF
    val frame1: DataFrame = spark.createDataFrame(newRdd, resSchema)

//    写入mysql
    frame1.write
      .format("jdbc")
      .option("url", "jdbc:mysql://localhost:3306/znh_text")
      .option("dbtable", "taxianalysisrunner")
      .option("user", "root")
